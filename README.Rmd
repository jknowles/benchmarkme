...
```{r}
res = benchmark_std(runs = 3)
res_p = benchmark_std(runs = 3, parallel = c(1, 2, 4))
```
...
bench_sum <- res_p %>% group_by(test, cores) %>% 
  summarize(elapsed = mean(elapsed), 
            test_group = first(test_group)) %>% 
  ungroup %>% 
  group_by(test) %>% 
  mutate(solo = elapsed[cores == 1]) %>% 
  mutate(speedup = round(solo / elapsed, 3))

ggplot(bench_sum, aes(x = cores, y = speedup, group = test, color = test)) + 
  geom_line() + theme_bw() + geom_point() + facet_wrap(~test_group) + 
